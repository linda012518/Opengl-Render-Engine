...
namespace lyd
{
	...
	void Shader::setMVP(matrix4& model, matrix4& view, matrix4& project)
	{
		setMat4("model", model);
		setMat4("view", view);
		setMat4("projection", project);
	}

	void Shader::setCameraPos(float3& cameraPos)
	{
		setVec3("viewPos", cameraPos);
	}

	void Shader::setColor(float4& color)
	{
		setVec4("inColor", color);
	}
	void Shader::setColor(float r, float g, float b, float a)
	{
		setVec4("inColor", r, g, b, a);
	}
}